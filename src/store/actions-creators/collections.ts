...
export const searchCollections = (serachString: string, page: number = 1, size: number = 10) => {
    return async (dispatch: Dispatch<CollectionsAction>) => {
        try {
            dispatch({
                type: CollectionsActionTypes.FETCH_COLLECTIONS
            ...
            const paginatedList = await apiClient.searchCollections(serachString, page, size);
